<script lang="ts">
	...
	async function onAmountClick(row: any, period: string) {
		try {
			$loading = true;
			transactionsList = await comet.finance.transactions.accountTransactions({
				...
				<tr>
					...
					{#each report.headers as header}
						<td data-label={header} class="text-end">
							<a href="#" class="link-dark" onclick={() => onAmountClick(row, header)}
								>{formatNumber(row[header])}</a
							>
						</td>
					{/each}
