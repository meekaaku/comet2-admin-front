...
class Rules {
	...
	async list(spec: TListSpec): Promise<RPaginated<RRule>> {
		...
		const response = await this.client.get(
			`finance/rules/list?page=${page}&page_size=${page_size}&sort=${sort}&filters=${jfilters}`
		);
...
class Transactions {
	...
	async accountTransactions(spec: any): Promise<RPaginated<RAccountTransaction>> {
		...
		const response = await this.client.get(
			`finance/transactions/account-transactions?account_id=${account_id}&period=${period}&${job_id_str}`
		);
